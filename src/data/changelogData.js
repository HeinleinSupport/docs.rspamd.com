    ...
    "sections": [
      ...
      },
      {
        ...
        "items": [
          ...
          "**ARC_ALLOW_TRUSTED scored at most once**: The symbol could be inserted multiple times for a single message, multiplying its score; it is now added at most once ([#6152](https://github.com/rspamd/rspamd/pull/6152))",
          "**History reset clears every row**: Two separate bugs left rows behind. The legacy in-memory ring buffer zeroed only the rows up to the current position and never reset the cursor, so once the buffer had wrapped, the tail kept completed entries that the read path still displayed; all rows are now cleared and the cursor reset. The Redis backend used `LTRIM <key> 0 0`, which retains list index 0 — the newest entry, since rows are prepended — so the most recent row always survived a reset; the list is now deleted outright, with a functional test covering the whole flow ([#6185](https://github.com/rspamd/rspamd/pull/6185), [#2910](https://github.com/rspamd/rspamd/issues/2910))",
          "**WebUI searches symbol option values again**: The Tabulator global search built its haystack from symbol name and description only, dropping the option values rendered in the symbols cell — a 4.1.4 regression that broke searching for values such as `asn:205640`. Options are included in the haystack again, restoring the old FooTable behaviour for both the history and scan tables ([#6169](https://github.com/rspamd/rspamd/pull/6169), [#6168](https://github.com/rspamd/rspamd/issues/6168))",
          "**Logger format string and argument mismatches**: Eight logger calls across lualib and the Lua plugins supplied a different number of arguments than their format string consumed. Most only produced extra-argument noise, but three misreported or lost information: the clustering plugin's Redis success message printed a literal \"unknown error\", the neural module printed a rule name where a value belonged and dropped the value itself, and lua_stat dropped the actual Redis error from a metadata conversion message ([#6177](https://github.com/rspamd/rspamd/pull/6177), [#6174](https://github.com/rspamd/rspamd/issues/6174))"
        ]
      },
      {
        "title": "Security",
        "items": [
          "**File, Path and Shm message sources gated by transport**: The `File`/`Path`/`Shm`/`Shm-Offset`/`Shm-Length` headers, their v3 metadata equivalents and the proxy's `File` query argument make rspamd open a path or map a shared memory object that the client names, and they were accepted on any transport — so any client able to reach a scanner, controller or proxy TCP port could have an arbitrary file readable by the rspamd user parsed and its content reported back. A new per-worker `allow_file_and_shm_inputs` boolean now gates the capability, derived solely from the transport the connection was accepted on: unix sockets always allow these inputs, since access is already controlled by the socket's filesystem permissions, while nothing the client sends takes part in the decision and no other credential — controller password, enable_password, secure_ip match, encrypted connection — unlocks it. Gating is applied before the named object is opened, mapped or statted, and two bypasses are closed: query arguments that the proxy turned into trusted upstream headers, and the fuzzy controller endpoints that build their own task. The inputs themselves are hardened — paths validated before any syscall, files snapshot with bounded reads instead of parsing a shared mapping the caller can truncate mid-parse, shared memory mapped as a page-aligned window rather than the whole object — the proxy strips client-supplied Shm headers as reserved hop-by-hop headers, the scanner's `max_tasks` now counts accepted body-pending connections, and the controller and proxy gain `max_connections`/`max_connections_per_source` admission limits enforced before any expensive allocation. `encrypted_only` also no longer exempts peers via a locality test that matched IPv6 link-local and site-local addresses, which had let any host on the segment skip the encryption requirement. The option defaults to true in this release with a prominent startup warning per TCP listener, and becomes false in the next major release ([#6162](https://github.com/rspamd/rspamd/pull/6162))",
          "**Controller authentication failures rate-limited per source**: Verifying a controller password runs a deliberately expensive key-derivation function (~25 ms for the catena hash that `rspamadm pw` emits by default) synchronously in the controller's event loop, and the password cache keys on the plaintext that succeeded — so a stream of distinct wrong passwords fell through to the KDF every time, and roughly 40 attempts per second were enough to saturate a single-process controller and stall every other request it serves. Failed attempts now feed a per-source leaky bucket checked before any password work, so a source over budget gets a 429 in well under a millisecond; a successful login clears the bucket, trusted sources (secure_ip, unix sockets) are never throttled, and throttled requests add no further penalty, so a shared source cannot be pushed into a permanent lockout. Configurable via `max_auth_failures` (default 10, 0 disables) and `auth_failure_window` (default 60 s); note the key is the real TCP peer, so behind a trusted reverse proxy all clients share one bucket. The redundant second KDF run when `password` and `enable_password` are configured to the identical hash is also skipped, halving the cost of every wrong password ([#6162](https://github.com/rspamd/rspamd/pull/6162))"
        ]
      }
    ],
    "summary": "This release closes a long-standing gap in the scan protocol — the File, Path and Shm message sources were accepted from any TCP client, letting anyone who could reach a scanner, controller or proxy port have arbitrary readable files parsed — behind a new transport-derived `allow_file_and_shm_inputs` option, and rate-limits the controller's expensive password verification per source. Fuzzy checks gain a privacy-conscious sender-facts extension over encrypted rules, riding on a wire format made forward compatible so new extension types no longer break older storages, while per-key fuzzy ACLs once again replace the global default as documented. PDF text extraction becomes font-aware, decoding through each font's /Encoding and /ToUnicode CMap instead of guessing over raw character codes, and the hyperscan cache finally works against replicated Redis. Recommended upgrade for all users; operators exposing scanner, controller or proxy ports beyond localhost should review the new `allow_file_and_shm_inputs` and connection-limit options now, ahead of the default flip in the next major release."
  },
  {
    "version": "4.1.4",
    "date": "2026-07-29",
    "type": "patch",
    "title": "Patch Release Fixing a Controller Authentication Fail-Open, Regexp Engine Resource Bounds, and jemalloc Startup Segfaults",
    "sections": [
      {
        "title": "Added",
        "items": [
          "**Read-only access to the Selectors tab**: The `list_extractors`, `list_transforms` and `check_selector` endpoints are now open to read-only users, matching the policy already used for maps — they only return static catalogs or parse a selector without executing it. `check_message` remains privileged, since it runs a user-supplied selector against a user-supplied message ([#6157](https://github.com/rspamd/rspamd/pull/6157))",
          "**Errors history for read-only users**: The `/errors` endpoint and its WebUI table were gated behind the privileged password. Any authenticated user can now view them: the endpoint returns only Rspamd's internal operational error log (timestamp, pid, level, module, message) — no message content, PII or secrets — while read-only users already see strictly more sensitive data in the History tab. Authentication is still required and the endpoint mutates nothing ([#6156](https://github.com/rspamd/rspamd/pull/6156))"
        ]
      },
      {
        "title": "Changed",
        "items": [
          "**One shared jemalloc instance per process** (incompatible): The build system searched for `jemalloc_pic` before `jemalloc`, so on Debian it preferred the static `libjemalloc_pic.a` over the shared library and linked it into the common library list used by nine shared objects. Each of them ended up with a complete private allocator holding its own arenas and extent map, so memory obtained through one copy could not be released through another — and jemalloc-enabled packages segfaulted at startup. CMake now searches for the shared library only, fails the configure step when just a static archive is available (except in fully static builds), and attaches jemalloc per target: `librspamd-server` — the only code calling `mallctl()` — plus each executable, pinned with `--no-as-needed` so the linker keeps the otherwise-unreferenced dependency and libc cannot win the `malloc()` lookup. The `malloc_conf` tuning moves into the server library, where it actually takes effect with a shared jemalloc (an executable exports nothing without `-rdynamic`, so it had been silently lost) and now applies to every rspamd binary rather than just the daemon. The jemalloc configuration is also built and tested in CI now, mirroring the shipped Debian/Ubuntu amd64 packages, which had never been covered before ([#6155](https://github.com/rspamd/rspamd/pull/6155), [#6153](https://github.com/rspamd/rspamd/issues/6153))"
        ]
      },
      {
        "title": "Fixed",
        "items": [
          "**PCRE2 match data leaked on invalid UTF input**: `rspamd_regexp_search()` allocates the PCRE2 match data before the JIT UTF-8 validation guard, but the early return on invalid input skipped the free at the function tail, leaking one match-data block (sized to the capture count) per rejected input. The guard is entered by any ordinary regexp — it tests whether a separate raw pattern was compiled, not whether raw matching was actually selected — and the regexp cache deliberately routes raw matching of UTF-marked classes through PCRE, so a message with a non-UTF-8 text part reached the leak through the normal scan pipeline, once per such regexp per part",
          "**Heap used by a single regexp match bounded**: PCRE2 match contexts set only the backtracking and depth limits, leaving the heap limit at the build-time default of 20 GB — effectively no limit. The interpreter grows its backtracking frame vector as frame size times depth, so one non-JIT match on a 60 KB subject with a 2 KB frame could allocate around 485 MB before the depth limit intervened, and far more with a larger frame; this path is taken whenever JIT is unavailable or refused for a pattern. An explicit 64 MiB heap limit is now set, preferring `pcre2_set_depth_limit` over the deprecated recursion-limit call where available. The PCRE1 path had no explicit limits at all — they live in the `pcre_extra` block, and `pcre_study` legitimately returns NULL when there is nothing to optimise — so a block is now allocated when needed and both match and recursion limits are set, with a much lower depth since PCRE1 backtracks on the C stack. Two PCRE1 drive-bys: study data is freed unconditionally rather than only in JIT builds, and the raw pattern is now studied for the raw matcher instead of handing it study data from a different compiled pattern",
          "**re:matchn() no longer loops forever on empty matches**: An incremental search resumes from the previous match end, so a zero-width match leaves the cursor where it was. `re:search()`, `re:split()` and the regexp cache loop all guard against this, but `re:matchn()` did not — with unlimited max matches, a pattern that can match the empty string spun forever and hung the worker. This was reachable from configuration: the spamassassin plugin passes unlimited for any `multiple` rule without `maxhits`, so one SA rule with an empty-matchable pattern wedged the process. The same cursor-progress guard is now applied",
          "**Bounded regexp patterns read past their end when generating ids**: `rspamd_regexp_new_len()` parses within the given pointer and length, but id generation then called `strlen()` on the same pointer. Callers passing a slice of a larger buffer — the composites expression parser and the symcache delayed symbols — read past the end of the pattern, a heap-buffer-overflow under ASAN whenever the slice is not NUL-terminated. The identity was wrong even when the read stayed in bounds: the same regexp got different ids as a slice and as a standalone string, while two different slices sharing a start address got the same id and collided in the regexp cache, which compares ids only. The length is now passed down to id generation, and three error paths that printed the raw pointer are bounded as well. Ids produced by `rspamd_regexp_new()` and the regexp cache are unchanged",
          "**Regexp data limit propagated to named scopes**: `regexp.max_size` was applied to the default scope only, so regexps registered in a named scope — multimap `regexp_rules`, for instance — were matched against unbounded input while the default scope was capped at 1 MiB. Named scope caches are now created inheriting the default scope limit, and setting the limit updates every scope registered so far, which covers both orderings: Lua scopes registered before the regexp module is configured, and plugins registering scopes during filters init",
          "**Lua URL filter consulted at most twice per user field**: Once the user field of a URL exceeded `max_email_user`, the parser consulted the Lua URL filter on every subsequent byte, and each call re-scanned the whole prefix — control characters, UTF-8 validation, `@` counting, custom filters. The 512-byte user limit in Lua never applied here because the mid-parse fragment contains no `@` yet, so this ran up to the 16 KB threshold: quadratic scanning plus thousands of C-to-Lua transitions, turning a single 16 KB URL into ~0.6 s of CPU. The filter is now consulted at most twice per user field — once when the limit is first exceeded, and once when the field ends at `:` or `@` so custom filters still see the complete userinfo. Parsing results are unchanged; the same URL now takes about a millisecond",
          "**Words retained for a message bounded by a message-wide budget**: The UTF tokenizer marked decayed words as skipped but still added them to the words vector, unlike the raw tokenizer which drops them — so the decay bounded nothing. Every word of a text part was retained at 72 bytes each and then normalized and stemmed with several pool allocations per word: a single 5 MB text part of 500k words cost 34 MiB of word vectors plus 27 MiB of pool allocations where the decay is meant to keep around 1200 words. The 200 ms tokenization guard did not help either, since it was only enabled for parts above 1 MiB, so a message of many sub-megabyte parts could accumulate huge word vectors within the 50 MiB message limit. Decayed words are now dropped as the raw tokenizer does; a message-wide budget (100k words, 1 MiB of text) is shared by all text parts and the meta words, with parts that hit it flagged as truncated and the message logged once; the time checks also trigger on the number of produced words; and skipped words are no longer normalized or stemmed. Note that decayed words of UTF parts are no longer visible to word regexp classes and part similarity hashes, which matches the long-standing raw tokenizer behaviour",
          "**CSS tokeniser stack overflow on sequential comments**: `next_token()` called itself after each comment, so an input made of many comments in a row consumed stack proportional to their count. The parser's own recursion limit does not cover this path, and the self-call is only elided when the compiler applies the tail call — a debug or ASAN build keeps a real call and overflows the stack. The tokenisation loop is now iterative, continuing from the offset past each comment, with doctest coverage for leading, trailing, interleaved, nested and unterminated comments plus a 100k-comment stress case",
          "**actrie startup segfault with jemalloc**: `acism_create()` allocated its buffers with GLib but released them with libc `free()`, which only works while both resolve to the same allocator. With the duplicated per-object jemalloc copies described above, `free()` inside the actrie library ran one jemalloc copy's deallocator on a pointer owned by whichever allocator GLib had bound to, segfaulting inside `rspamd_url_init()` during startup. The whole lifecycle now stays inside GLib, so the pair resolves to one allocator no matter how symbol interposition falls out ([#6153](https://github.com/rspamd/rspamd/issues/6153))",
          "**rspamadm resolves SRV-based upstreams**: rspamadm never configured the upstreams library, so the upstreams context had neither an event loop nor a resolver. Plain host names still worked, as they resolve synchronously during config parsing, but the SRV form `servers = \"service=fuzzy+example.com\"` creates a placeholder that is only selectable after asynchronous resolution — which was never scheduled. Every fuzzy storage configured that way looked empty: `rspamadm fuzzy_ping -l` printed no servers and pinging failed with \"no fuzzy storage upstream available\". The library is now configured right after the resolver is created, before any command loads its configuration, and since one-shot commands issue their first request before the resolve timer fires, a new `lua_fuzzy.wait_for_storages()` — used by `fuzzy_ping` and `fuzzy_hash` — waits for the selected rules to have servers, bounded by the request timeout; an explicit `-s`/`--server` override skips the wait as it bypasses the configured upstreams. A ping that cannot start now also reports the rule name instead of nil"
        ]
      },
      {
        "title": "Security",
        "items": [
          "**Controller fails closed on a malformed password hash**: `rspamd_is_encrypted_password()` validated only the `$<id>` prefix of a configured password, not that a salt and a key follow it. A hash such as `$1$` therefore passed the check, reached the verification with both components absent, skipped the comparison block entirely and returned its initial value of TRUE — any supplied password authenticated, and was then cached as the valid one. The result is now initialised to FALSE and set only after a completed KDF comparison, and the malformed hash is logged instead of silently accepted. The component extraction also indexed three bytes into the string without checking its length, reading past the terminator for short inputs and making the fail-open depend on adjacent heap bytes; it now takes the length and bails out when a component is missing. The startup sanity check re-ran the same prefix-only test, so a malformed password also passed configtest silently — it now decodes the salt and the key and validates their lengths against the pbkdf matching the hash id, which additionally fixes a hardcoded reference to the first pbkdf in the list"
        ]
      }
    ],
    "summary": "This release closes a critical controller authentication fail-open — a malformed password hash such as `$1$` accepted any password, and configtest did not catch it — and fixes the startup segfault in jemalloc-enabled packages by linking one shared allocator instance per process instead of a private copy per shared object, a configuration that is now also exercised in CI. The regexp engine gets a broad hardening pass: an explicit heap bound for a single match on both PCRE2 and PCRE1, a match-data leak on invalid UTF input reachable from the normal scan pipeline, an infinite loop in `re:matchn()` on empty-matchable patterns, an out-of-bounds read when generating ids for bounded patterns, and the data limit finally applied to named scopes. Rounding it out: quadratic URL parsing with per-byte Lua filter calls is gone, tokenizer word retention is bounded by a message-wide budget, the CSS tokeniser no longer overflows the stack on comment floods, and rspamadm handles SRV-based fuzzy upstreams. Recommended upgrade for all users, and strongly recommended for anyone running the controller with worker passwords."
  },
  {
    "version": "4.1.3",
    "date": "2026-07-25",
    "type": "patch",
    "title": "Hardening Release with DKIM/SPF Correctness Fixes, Parser Resource Bounds, Fuzzy Hash Observability, and a jQuery-Free WebUI",
    "sections": [
      {
        "title": "Added",
        "items": [
          "**Structured fuzzy match results and a diagnostics API**: Every fuzzy match now produces a structured record — rule, symbol, upstream, stored and queried digests, match type, probability, score, flag, value and hash timestamp — kept in the `fuzzy_matches` mempool variable and exposed to Lua via `task:get_fuzzy_results()`. All queried hashes, misses included, are recorded in `fuzzy_checked`. The symbol option for non-exact matches now carries both hash prefixes (`flag:found:prob:type:queried`), so a match is diagnosable from a scan report alone. The storage sets a flag bit in the first reserved byte of the reply when the digest field holds the digest actually resolved by the backend, and the client propagates it as a `confirmed` field, removing the guesswork about echoed queries when talking to legacy storages. A new `fuzzy_check.check(task, cb, rule, timeout[, hashes][, server])` Lua API queries a storage either with the hashes generated from the task message (shingles included) or with an explicit list of hex digests, reporting per-reply results including misses and error codes",
          "**Configurable fuzzy probability weight curve**: The score multiplier for non-exact matches used to be `sqrt(prob)`, a concave curve anchored at zero. Since the storage never returns a probability below the shingle match threshold of 0.5, the whole reachable range collapsed into roughly (0.73 .. 1.0], so a marginal 17/32 shingle overlap scored almost as high as an exact match — combined with high-weight deny lists this turned weak matches into instant rejects. The curve is now `((prob - prob_bias) / (1 - prob_bias)) ^ prob_power`, anchored at the match threshold (`prob_bias` 0.5). With the default `prob_power` of 1.0 the multiplier is simply the fraction of the way from the threshold to an exact match (19/32 → 0.19, 24/32 → 0.50, 28/32 → 0.75); raise it per rule for harsher discounting of weak matches. The old zero-anchored curve is not preserved. Applies to text and HTML hashes; small images keep their existing normalized curve",
          "**Persistent shingle sets in the redis fuzzy backend**: Shingle key suffixes are now stored as the `S` field of the digest hash on every ADD, making each digest self-describing and closing two long-standing blind spots. Digest-only deletions (for example periodic deletions driven by a hash list) previously left the digest's shingle slots orphaned; they are now reconstructed from the persisted set and removed when still owned by the deleted digest. REFRESH used to touch the shingle slots of the message that happened to trigger it — a different message by definition — and now refreshes the slots the digest still owns and reclaims the vacant ones, so a hash that keeps matching keeps its full shingle anchor instead of decaying. The sqlite backend needs no changes, since its shingles table already cascades on delete",
          "**Per-hash storage introspection**: `rspamadm control fuzzyhash <128 hex digest>` asks all fuzzy workers for storage-side diagnostics of a specific hash: flag slots and values, creation time, remaining TTL and — using the newly persisted shingle set — slot ownership (owned/foreign/vacant out of total). Ownership directly answers the central false-positive triage question: can this hash still produce fuzzy matches, or has its shingle anchor decayed? The redis backend gathers everything in a single server-side script round trip; sqlite queries the digests and shingles tables synchronously",
      ...
      },
      {
        ...
        "items": [
          ...
          "**One bounded zstd decompression helper across HTTP, proxy and maps**: The streaming decompression loop was copy-pasted in seven places with inconsistent bounding. The proxy had no output ceiling at all — on request and response bodies and on the v3 multipart result and body parts, a zstd bomb from a client or backend could balloon memory without limit. The task and v3 protocol paths were bounded but enforced the limit only when the buffer filled, letting output overshoot `max_message` up to twofold, and never validated the final length; the map shm-cache, file-cache and static paths grew their buffers with no ceiling. A single `rspamd_zstd_decompress_bounded` now validates the frame-advertised size before any allocation, caps buffer growth strictly at the limit, drains pending decoder output and frees the buffer on every error path. All seven sites are converted: `max_message` bounds the task, protocol v3 and proxy paths, `max_map_size` the map cache and static paths",
          "**HTTP request bodies bounded on the controller, proxy and control sockets**: The normal worker caps HTTP bodies at `max_message`, but the controller router, the proxy client connection and the main control socket never set a maximum — a declared Content-Length caused an unbounded preallocation and chunked input could grow until the connection ended, all before routing or authentication. The HTTP router gains a `max_size` knob applied to every accepted connection, with `max_message` set as the cap on all three; oversized requests are rejected with 413 on the controller and a closed connection on the proxy",
          ...
          "**PDF text line breaks**: The Td/TD text positioning operators never produced a newline — the grammar did not capture the operator name, so the handler compared a numeric operand against the string `Td` (never true) and additionally read the wrong operand. Consecutive text lines were therefore concatenated, so for example \"…June 1, 2026.\" followed by \"You may obtain…\" became \"2026.You\", which the URL parser mis-detected as `http://2026.you` and raised false positives. The operator is now captured and a newline rendered when the vertical displacement is non-zero",
          "**Duplicate module sections warned about**: `rspamd_config_get_module_opt()` returns the rule or option object from the first section only when a module section is duplicated at the top level — for example by a stray file in `modules.d` — so the remaining sections are silently ignored and their rules are never loaded. An explicit warning is now emitted per configured C module, so configtest and the startup log surface the problem instead of silently dropping rules. Lua modules are unaffected, since they flatten the whole duplicate chain",
          "**WebUI: malformed /stat response routes to the login dialog**: The connect probe swallowed JSON parse errors, so a 2xx `/stat` with a non-JSON body — truncated by a proxy, a captive-portal page, corruption — loaded an empty, half-loaded main UI with a \"Cannot get server status\" alert and no login path. The body is now parsed explicitly and any failure shows the connect dialog, matching the pre-migration behaviour. The unreachable 304 success branch inherited from jQuery's default success range is also removed, since the controller sends `Cache-Control: no-store` and never emits Last-Modified or ETag for these endpoints",
    ...
    "sections": [
      {
        ...
        "items": [
          ...
          "**rspamadm password from the environment**: The controller password can be supplied via the environment instead of an interactive TTY prompt, so automation (for example, a continuous-deployment pipeline verifying a clean state) can drive password-protected `rspamadm` operations non-interactively ([#6114](https://github.com/rspamd/rspamd/pull/6114))",
      ...
      },
      {
        ...
        "items": [
          ...
          "**Glob map patterns anchored to the whole subject**: Glob map entries were compiled into unanchored regexps with substring-search semantics, so a `t.co` entry matched `walmart.com` and `*.bit.ly` matched `foo.bit.ly.evil.com`. Translated patterns are now wrapped in `^(?:...)$` at map load time: bare names match exactly, wildcards match only what they say. This affects all glob maps — multimap `glob`/`glob_multi`, url_redirector `redirector_hosts_map`, dkim_signing/arc signing and key tables, mx_check exclusions, and the rbl glob returncodes matcher. Maps that relied on the accidental substring behaviour must now use explicit wildcards ([#6125](https://github.com/rspamd/rspamd/issues/6125))",
          "**Selectors must parse to the end of input**: The selector grammar was not anchored, so lpeg matched the longest valid prefix and silently dropped the rest — `from(\"smtp\"):domain:lower` was accepted and evaluated as `from(\"smtp\"):domain`, and trailing garbage after a selector or a `;` list element was ignored across every consumer, including the WebUI's selector checker. Parsing now requires the whole input to be consumed and reports the position and the unparsed tail otherwise. Selectors that previously loaded thanks to the silent truncation now fail configuration load with a pointed error; they were never evaluating as written, so failing loudly is the correct behaviour ([#6127](https://github.com/rspamd/rspamd/pull/6127))",
          ...
          "**Duplicate GPT request with synchronous context_augment**: When a `context_augment` function invoked its callback synchronously, the pending-fetches counter dropped to zero inside the callback and triggered the request; control then returned to the main flow, where a now-redundant guard fired a second request — this time without the context snippet. The duplicate path is removed ([#6142](https://github.com/rspamd/rspamd/pull/6142))",
          "**ICAP HTTP header insertion**: Fixed incorrect insertion of an HTTP header when the ICAP module assembles the request headers in `get_http_headers` ([#6117](https://github.com/rspamd/rspamd/pull/6117))",
          "**dmarc_report batch throttling**: Report sending no longer calls a blocking sleep from an async callback when throttling batches, which stalled the event loop instead of pacing the sends ([#6132](https://github.com/rspamd/rspamd/pull/6132))",
          "**Lua 5.5 compatibility**: Lua 5.5 makes `for`-loop control variables read-only, turning assignments to them into compile-time errors. The most visible casualty was the bundled lupa Jinja template engine, which failed to load with \"attempt to assign to const variable 'word'\" and made rspamd unable to start when configs use Jinja templating. All remaining offenders across the tree — found by parse-checking with luac 5.5 — now rebind the value to a local instead ([#6130](https://github.com/rspamd/rspamd/pull/6130))"
        ]
      },
      {
        "title": "Security",
    ...
    "sections": [
      {
        ...
        "items": [
          ...
          "**Explicit multipattern SOM flag**: `RSPAMD_MULTIPATTERN_SOM` (`rspamd_trie.flags.som`) explicitly requests start-of-match offsets, overriding the incompatible single-match flag. The offset convention is now documented — 1-based pattern id, 0-based byte offsets, start inclusive and end exclusive (so `end - start` is the match length) — and the regex fallback used when hyperscan is unavailable now reports the true start/end from the PCRE search instead of a bogus `end - strlen(pattern)` for variable-length matches ([#6105](https://github.com/rspamd/rspamd/pull/6105))",
      ...
      },
      {
        "title": "Fixed",
        "items": [
          "**DNS resolver startup crash (4.1.0 regression)**: A nameserver that failed to resolve at config time was turned into a zero-address pending-resolve upstream and then dereferenced as NULL in `rspamd_dns_server_init`, crashing the worker with SIGSEGV at startup. Resolver nameservers are consumed synchronously and can never be promoted asynchronously, so they are no longer deferred, with a NULL guard added as defense in depth ([#6096](https://github.com/rspamd/rspamd/issues/6096))",
          "**Milter QUARANTINE with custom reply (3.10.0 regression)**: The QUARANTINE command was emitted only inside the branch that synthesises a default quarantine reason, so a caller-supplied SMTP message (e.g. `task:set_pre_result('quarantine', 'reason')`) suppressed the command entirely and the message was accepted instead of quarantined. The supplied message now becomes the quarantine reason, matching the reject and tempfail branches; this affects both `METRIC_ACTION_QUARANTINE` and reject converted via `quarantine_on_reject` ([#6088](https://github.com/rspamd/rspamd/issues/6088))",
          "**Coroutine thread-pool safety**: Async Lua libraries (DNS, Redis, TCP, HTTP, util) capture the running coroutine at a yield point and resume it later from a C completion callback, with nothing previously guaranteeing the resumed entry was still the captured one. Each pooled thread now carries an explicit lifecycle and generation counter, and the entries are refcounted, so a double-fired event, a completion racing task teardown, or an entry recycled into another task either resumes the correct live coroutine or becomes a logged no-op — instead of dereferencing freed memory ([#6104](https://github.com/rspamd/rspamd/pull/6104))",
          ...
          "**URLs with multiple '@' and backslashes**: URLs containing several `@` signs and backslashes were multiplied into many spurious URLs. Long legitimate conversations can accumulate such links, so the spurious expansion could drop mail and, with autotrain enabled, train the system badly; those URLs are no longer multiplied ([#6065](https://github.com/rspamd/rspamd/pull/6065))",
    ...
    "sections": [
      ...
      },
      {
        ...
        "items": [
          ...
          "**Chain-aware URL redirector cache**: `url_redirector` gains a per-hop Redis cache with intermediate-hop injection, so shared intermediate links are reused across chains and resolved hops are made visible to downstream modules; the walk self-heals on partial cache misses and surfaces previously-silent lock/stale conditions ([#6014](https://github.com/rspamd/rspamd/pull/6014))",
          "**Stealth fingerprint profiles**: `url_redirector` replaces its flat `default_ua` list with five coherent browser profiles (Chrome, Edge, Firefox, Safari), each bundling a User-Agent with the exact header set, values, and order that browser sends (including `sec-ch-ua` client hints where appropriate). One profile is picked per task and reused across every hop ([#6053](https://github.com/rspamd/rspamd/pull/6053))",
          "**Glob redirector hosts**: `redirector_hosts_map` now accepts glob patterns (e.g. `*.bit.ly`, `*.t.co`); bare hostnames still match exactly, so existing maps are unaffected ([#6056](https://github.com/rspamd/rspamd/pull/6056))",
          "**GET for selected redirector URLs**: `url_redirector` can issue GET (rather than HEAD) for an operator-defined list of URLs matched by regexp ([#6043](https://github.com/rspamd/rspamd/pull/6043))",
          ...
          "**Richer Elasticsearch logging**: The `elastic` module now logs Reply-To user/domain, received IPs, per-URL and CTA URL metadata via a new `collect_urls` block, and the forcing module name from `task:has_pre_result()` ([#6018](https://github.com/rspamd/rspamd/pull/6018))",
          "**ClickHouse column presets**: Named `extra_columns` presets, including an initial outbound preset, can be selected without hand-rolling the schema ([#5983](https://github.com/rspamd/rspamd/pull/5983))",
          "**New selectors**: `fuzzy_digest`, `fuzzy_shingles`, `authenticated`, and `received_count` ([#5981](https://github.com/rspamd/rspamd/pull/5981))",
          ...
          "**DMARC report throttling**: `rspamadm dmarc_report` gains `-w`/`--batch-wait` to pause between batches (and stagger report generation), avoiding overload of the SMTP server and a weak resolver ([#5985](https://github.com/rspamd/rspamd/pull/5985))",
          "**autolearnstats sorting**: `--sort-by <col>` and `--group` options for the `rspamadm autolearnstats` table output ([#6050](https://github.com/rspamd/rspamd/pull/6050))",
          "**Feedback report parsers**: New `lua_feedback_parsers` lualib for parsing DSN (delivery status) and ARF (abuse) reports ([#5982](https://github.com/rspamd/rspamd/pull/5982))",
          "**Structured custom-Lua loader**: `lua_extras` provides a two-phase loader for custom selectors, maps, and regexps under `lua.local.d/{maps,selectors,regexps}/`, resolving cross-kind dependencies (maps → selectors → regexps) so a selector can consume a map registered by an earlier kind ([#6020](https://github.com/rspamd/rspamd/pull/6020))",
          "**eXpurgate scanner**: New `lua_scanners` engine for the eXpurgate anti-spam service ([#5755](https://github.com/rspamd/rspamd/pull/5755))",
          "**Per-worker memory dumps**: New `rspamadm control memstat` command reports per-worker RSS, per-callsite mempool counters, Lua heap usage, and structured per-arena jemalloc stats, with `--short`, `--sort`, and per-section toggle flags ([#6016](https://github.com/rspamd/rspamd/pull/6016))",
          "**Container-friendly baseline config**: The baseline pidfile and logging type/filename are now env-overridable (`RSPAMD_PIDFILE`, `RSPAMD_LOG_TYPE`, `RSPAMD_LOG_FILE`); an empty pidfile disables it (useful as PID 1), and `RSPAMD_LOG_TYPE=console` logs to stdout. Stock installs render the previous defaults bit-for-bit ([#6067](https://github.com/rspamd/rspamd/pull/6067))",
          "**Auto-loaded fasttext model**: When no `fasttext_model` is configured, the shipped `$SHAREDIR/languages/fasttext_model.ftz` is loaded if present, so images bundling the model can drop the explicit override; stock installs without the file behave exactly as before ([#6067](https://github.com/rspamd/rspamd/pull/6067))",
          "**Fixed-point float formatting**: `fpconv` gains `%.Nf` fixed-point formatting with correct rounding and carry handling ([#6061](https://github.com/rspamd/rspamd/pull/6061))"
        ]
      },
      {
        "title": "Fixed",
        "items": [
          ...
          "**Ratelimit multi-bucket tracking**: A selector rule with several buckets (e.g. `200/1h` plus `30/1m`) keyed every bucket on the selector value alone, so only the last bucket was tracked and the other limits were silently ignored; each bucket now gets a distinct Redis key ([#6076](https://github.com/rspamd/rspamd/pull/6076), [#6059](https://github.com/rspamd/rspamd/issues/6059))",
          "**Verbatim href preserved**: `url:get_raw()` returned the partially percent-decoded scratch buffer for HTML URLs; `url->raw` now points at a mempool-owned copy of the verbatim trimmed href ([#5986](https://github.com/rspamd/rspamd/issues/5986))",
          ...
          "**Deterministic ARC header order**: `lua_mime.modify_headers` honoured its `order` list but serialised in hash order; ARC sets are now emitted in the conventional ARC-Seal / ARC-Message-Signature / ARC-Authentication-Results layout, which some validators (e.g. O365) require ([#6052](https://github.com/rspamd/rspamd/pull/6052), [#6045](https://github.com/rspamd/rspamd/issues/6045))",
          "**DKIM permfail mapping**: An invalid DKIM record now maps to `dkim=permerror` in `Authentication-Results` instead of falling through to `dkim=none` ([#6028](https://github.com/rspamd/rspamd/pull/6028), [#5957](https://github.com/rspamd/rspamd/issues/5957))",
          "**EAI Message-ID handling**: The `INVALID_MSGID` rule now honours `mime_utf8`, and the `enable_mime_utf8` option spelling is registered as an alias so it actually takes effect, fixing false positives on valid SMTPUTF8 Message-IDs (RFC 6532) ([#6011](https://github.com/rspamd/rspamd/pull/6011), [#6007](https://github.com/rspamd/rspamd/issues/6007))",
          ...
          "**Fuzzy persistent-TCP leak**: Per-frame `cmd_session` state (extensions buffer, key refs) leaked on every frame after the first on a persistent TCP connection; per-command state caching is dropped so each frame starts clean ([#6001](https://github.com/rspamd/rspamd/issues/6001))",
          "**Allowed fuzzy clients on TCP**: Allowed clients are no longer blocked on the TCP path ([#5992](https://github.com/rspamd/rspamd/pull/5992))",
          "**Neural ANN survival**: A trained ANN is now preserved across symbol-list drift and symcache-driven profile rotation; the profile digest is stabilised under `disable_symbols_input` (keyed on the providers config rather than the unrelated symbol catalogue), and training is retargeted to the newest profile so inference no longer goes dark for weeks until a fresh model trains ([#6041](https://github.com/rspamd/rspamd/pull/6041))",
          "**Inline /checkv3 settings**: Inline `metadata.settings` on `/checkv3` were stashed directly on the task and skipped the apply pipeline; they now run through the same `settings.lua` apply path as v2, so action thresholds, symbols, subject, variables, and header edits take effect ([#5999](https://github.com/rspamd/rspamd/issues/5999))",
          "**/checkv3 request headers**: Arbitrary client HTTP headers are now registered so `task:get_request_header()` works under v3, restoring v2 behaviour ([#5998](https://github.com/rspamd/rspamd/issues/5998))",
          ...
          "**Lua TCP leak**: A connection that read without writing leaked; the leak is closed ([#6048](https://github.com/rspamd/rspamd/pull/6048))",
          "**Redis master under Sentinel**: Standalone `rspamadm` tools never resolved the current Redis master, round-robining writes that failed `READONLY` on replicas; a new one-shot `lua_redis.prepare_redis_setup` resolves the master (and loads scripts) for tools like `dmarc_report` ([#6015](https://github.com/rspamd/rspamd/pull/6015), [#6009](https://github.com/rspamd/rspamd/issues/6009))",
          ...
          "**Greylist timeout separation**: The greylisting period (5 min) was being picked up by `lua_redis` as the Redis connection timeout; a separate `redis_timeout` (default 1.0s) is introduced and propagated into nested `redis{}` blocks ([#5977](https://github.com/rspamd/rspamd/pull/5977))",
          "**rspamadm vault output**: `rspamadm vault list` produced empty output for large vaults because the payload passed through a format-string logger; it is now written to stdout directly via `io.write` ([#6006](https://github.com/rspamd/rspamd/pull/6006), [#6005](https://github.com/rspamd/rspamd/issues/6005))",
          "**Regexp capture groups**: An empty capture group in the middle of a match no longer discards all following groups (PCRE1 and PCRE2); results are truncated at the last non-empty group instead ([#5974](https://github.com/rspamd/rspamd/pull/5974), [#5973](https://github.com/rspamd/rspamd/issues/5973))",
          "**x-binaryenc charset**: ICU conversion is skipped for the synthetic `x-binaryenc` charset across all detection paths, silencing the spurious \"cannot open converter\" warning and correctly marking the part as raw binary ([#5984](https://github.com/rspamd/rspamd/pull/5984))",
          "**Timeout sanity check**: `configtest` now warns when `task_timeout` is less than a symcache symbol timeout, including per-worker overrides ([#5978](https://github.com/rspamd/rspamd/pull/5978))",
          "**libc++ builds**: Use `string_view::data()` for pointer access where libc++ returns a wrapped iterator from `begin()`, fixing builds with libc++ 22 on FreeBSD ([#5969](https://github.com/rspamd/rspamd/issues/5969))",
      ...
      },
      {
        ...
        "items": [
          ...
          "**Nested-query URL DoS**: A crafted message with a few levels of percent-escaped nested query URLs exhausted the multipattern hyperscan scratch pool and aborted the worker via assertion; the scratch budget is enlarged, the query-nesting cap is restored to a fixed functional limit, and scratch exhaustion is made non-fatal ([#6066](https://github.com/rspamd/rspamd/pull/6066))",
          ...
          "**Header lookahead over-read**: `rspamd_string_find_eoh` peeked `p[1]` guarded only by `p < end`, reading one byte past the buffer on input ending in `\\r\\r`; it now checks `p + 1 < end`",
