...
- `sender_key_size`: length of hash key for the global replies set key (default 20); per-sender local set keys always use a hard-coded 8-character hash and are not affected by this setting
- `message`: message passed as the pre-result reason when `action` is set (default `Message is reply to one we originated`)
- `symbol`: symbol yielded on messages identified as replies (default `REPLY`)
- `score`: default score for the symbol (default -4)
- `max_local_size`: maximum number of recipients stored in each per-sender local replies set (default is 20)
- `max_global_size`: maximum number of recipients stored in the global replies set (default is 30)
